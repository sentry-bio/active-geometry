...
theorem midpoint_exponent_eq (d r : ℝ) (hr : 1 < r) :
  ...
  have hsq : (√r) ^ 2 = r := sq_sqrt (le_of_lt hr0)
  have hlog : log (√r) = log r / 2 := log_sqrt (le_of_lt hr0)
  have hfactor : r - 1 = (√r - 1) * (√r + 1) := by nlinarith [hsq]
  have hne : √r + 1 ≠ 0 := by
  ...
  have hsm : √r - 1 ≠ 0 := by
    ...
    have : r = 1 := by
      calc
        ...
        _ = 1 := by simp [hsqrt]
    linarith
  have hquot : log r / (r - 1) * (√r - 1) = log r / (√r + 1) := by
    rw [hfactor]
    field_simp [hne, hsm]
    ring
...
theorem spanInformation_pos (d r : ℝ) (hd : 0 < d) (hr : 1 < r) :
  ...
  have hlogt : 2 * (logMean r - 1) / (logMean r + 1) < log (logMean r) := by
    simpa [sub_add_cancel] using lt_log_one_add_of_pos hx
  have hcmp :

...
def run_sweep(
    ...
    for h, s in raw:
        if len(s) >= 1000:
            s = s[:max_len]
            seqs.append(s)
            labels.append(int(hashlib.md5(s.encode()).hexdigest(), 16) % 5)
    print(f"Device: {device}")
    print(f"Loaded {len(seqs)} sequences; label counts: {dict(Counter(labels))}")
...
if __name__ == "__main__":
    ...
    p.add_argument("data", type=str, help="Path to multi-sequence FASTA")
    p.add_argument("name", type=str, help="Name for logging")
    args = p.parse_args()
    run_sweep(args.data, args.name)

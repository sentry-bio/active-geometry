...
class PoincareMapping(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:  # [B, *]
        ...
        # scale radius to stay inside ball
        r_max = 0.9 / torch.sqrt(self.c)
        z_euc = z_euc * r_max

        man = self._man()
        if man is not None:
            return man.projx(z_euc)  # type: ignore
        return z_euc
    ...
    def dist_mat(self, z: Tensor) -> Tensor:  # z: [B, d]
        man = self._man()
        if man is not None:
            return man.dist(z.unsqueeze(1), z.unsqueeze(0))  # [B, B]
        # Euclidean fallback
...
class BiosphereLoss(nn.Module):
    ...
    def __init__(self, manifold: PoincareMapping, mask_id: int, mlm_prob: float = 0.15, temp: float = 0.1):
        super().__init__()
    ...
    def forward(
        ...
        total = mlm_loss + dec_loss + 0.1 * hex_loss + 0.5 * dist_loss
...
class BiosphereCodec(nn.Module):
    ...
    def __init__(
        ...
        vocab: int,
        d_model: int = 512,
        n_layers: int = 6, max_len: int = 131_072,
        latent_dim: int = 128,
        ...
        # Core components
        self.encoder = BiosphereEncoder(vocab, d_model, n_layers, max_len)
